...
// BOOTLOADER Version
__attribute__((section(".VERSION.SGN"))) volatile const uint16_t VERSION = 3000;
...
BOOTLOADER_STATUS checkAppSign(uint32_t _sign, uint32_t _crc){
	...
	if((signature[0]=='E') && (signature[1]=='N') && (signature[2]=='K') && (signature[3]=='O')){
		...
		if( *(uint32_t *)(_crc) != getCRC((uint8_t*)0x8000000, _crc - 0x08000000)){
...
void sendTAG(void){
	...
	// Second and Third bytes are BL version
	sendByte((uint8_t)(VERSION >> 0));
	sendByte((uint8_t)(VERSION >> 8));
